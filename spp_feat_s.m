function [ feat,boxes_scales ] = spp_feat_s( im ,boxes,spm_im_size,averageImageV,net,spp_pooler)
...
for i = 1:length(spm_im_size)
    ...
    resized_im = imresize(im, [im_resized_height, im_resized_width], 'bilinear', 'antialiasing', false);
      ...
      input_im=gpuArray(single(input_im));
      res=vl_simplenn(net, input_im);
      % 2 or seven that's a big problem
      feat.rsp{i} = gather( res(end).x);
end
